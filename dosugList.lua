...
function scene:create( event )
    ...
    local newParams =  getText("1. Учитесь новому", 20, newTitleParams, 90)
    local summary = display.newText(newParams)
    summary:setFillColor( 0 ) -- black
    scrollView:insert( summary )

    local newParams =  getText("За прошедшее время самоизоляции и карантина у вас наверняка все запасы сладостей съедены, сериалы просмотрены, а кот в ужасе прячется от ваших попыток почесать ему пузико. Чем же заняться, чтобы не сойти с ума сидя дома и отвлечься от новостей про коронавирус? Мы нашли парочку идей!", 14, newParams, 90)
    local summary = display.newText(newParams)
    summary:setFillColor( 0 ) -- black
    scrollView:insert( summary )

    local newParams =  getText("Читать статью", 14, newParams, 90)
    local summary = display.newText(newParams)
    ...
    local newParams =  getText("Типы масок", 20, newTitleParams, 70)
    local summary = display.newText(newParams)
    summary:setFillColor( 0 ) -- black
    scrollView:insert( summary )
    ...
    local newParams =  getText("Домашние тренировки", 20, newTitleParams, 70)
    local summary = display.newText(newParams)
    summary:setFillColor( 0 ) -- black
    scrollView:insert( summary )
    ...
    local newParams =  getText("Антисептик?", 20, newTitleParams, 70)
    local summary = display.newText(newParams)
    summary:setFillColor( 0 ) -- black
    scrollView:insert( summary )
